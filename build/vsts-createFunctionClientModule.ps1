...
$parentModule = 'AzureADAuthMethods'
if (-not $ModuleName) { $ModuleName = 'AzureADAuthMethods.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AzureADAuthMethods'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AzureADAuthMethods.psd1"
$prereqHash = @{
